...
async fn setup_test_market(server: &TestServer, _market_id: &str) -> anyhow::Result<()> {
    ...
    // Create BTC token
    client
        .admin_create_token("BTC".to_string(), 6, "Bitcoin".to_string())
        .await?;
...
async fn test_bot_order_with_proper_rounding() {
    ...
    setup_test_market(&server, "BTC/USDC")
        .await
        .expect("Failed to setup market");
...
async fn test_bot_order_below_min_size_rounded_to_zero() {
    ...
    match result {
        Ok(placed) => {
            ...
            assert_eq!(
                placed.order.size, 0,
                "Order below min_size should be rounded to 0"
            );
        }
        Err(_) => {
...
async fn test_bot_order_displays_correctly() {
    ...
    let orders = client
        .get_orders(&user_address, Some(market_id))
        .await
    ...
    assert!(
        (displayed_price - 95000.0).abs() < 0.01,
        "Price display incorrect: {}",
        displayed_price
    );
    assert!(
        (displayed_size - 0.01).abs() < 0.000001,
        "Size display incorrect: {}",
        displayed_size
    );
}
...
async fn test_bot_order_with_fractional_price() {
    ...
    assert!(
        result.is_ok(),
        "Order should be placed with rounded price: {:?}",
        result
    );
    ...
    if let Ok(placed) = result {
        ...
        assert_eq!(
            placed.order.price, 95124000000,
            "Price should be rounded to $95,124, got: {}",
            placed.order.price
        );
    }
}
...
async fn test_bot_multiple_orders() {
    ...
    assert!(
        orders.len() >= 5,
        "Should have at least 5 orders, got: {}",
        orders.len()
    );
    ...
    for order in orders.iter() {
        ...
        assert_eq!(
            order.price % 1000000,
            0,
            "Price {} not rounded to tick_size",
            order.price
        );
...
async fn test_bot_cancel_all_orders() {
    ...
    let orders_before = client
        .get_orders(&user_address, Some(market_id.clone()))
        .await
        .expect("Failed to fetch orders");
