    ...
    c.bench_function("match_single_order_empty_book", |b| {
        b.iter(|| {
            ...
            let matches = rt.block_on(async {
                orderbook.match_order(&market, black_box(&order)).await
            });
            black_box(matches);
        });
    });
            ...
            |b, &size| {
                b.iter(|| {
                    ...
                    rt.block_on(async {
                        ...
                        let matches = orderbook.match_order(&market, black_box(&buy_order)).await;
                        black_box(matches);
                    });
                });
            },
    ...
    c.bench_function("market_order_execution", |b| {
        b.iter(|| {
            ...
            rt.block_on(async {
                ...
                let matches = orderbook.match_order(&market, black_box(&market_order)).await;
                black_box(matches);
            });
        });
    });
